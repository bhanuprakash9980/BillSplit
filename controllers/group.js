...
module.exports = (db) => {
        ...
        db.user.getSingleUserByName(valuesInBracs, (error, result2) => {
          ...
          if (result2) {
            ...
            db.group.userGroupLink(arrayOfId, (error, result3) => {
              if (result3) {
                db.main.updateActivityForCreateGroup(
                  user_id,
                  user_name,
                  'created',
                  group_id,
          ...
          db.group.getGroupCount(user_id, (error, result2) => {
            if (result2) {
              ...
              for (let i = 0; i < groupDetails.length; i++) {
                let members_net = netDetails
                  .filter((x) => x.group_id === groupDetails[i].id)
                  .map((x) => {
                    return {
                      name: x.name,
                      net: x.net,
                ...
                let user_net = members_net.reduce(
          ...
          if (result) {
            ...
            db.group.getSingleGroup(group_id, (error, result) => {
              if (result) {
                ...
                db.bill.getPaidSplitAmountAsPayer(
                  group_id,
                  user_id,
                  (error, result) => {
                    if (result) {
                      ...
                      db.bill.getPaidSplitAmountAsPayee(
                        group_id,
                        user_id,
                        (error, result) => {
                          if (result) {
                            data['settled_split_amount_as_payee'] = result;
                            db.bill.checkTrueCountOfBillsByGroup(
                              group_id,
                              (error, result) => {
                                if (result) {
                                  ...
                                  for (
                                    let i = 0;
                                    i < data.billList.length;
                                    i++
                                  ) {
                                    for (let j = 0; j < trueTable.length; j++) {
                                      if (
                                        data.billList[i].bill_id ===
                                        trueTable[j].bill_id
                                      ) {
                                        if (
                                          parseInt(trueTable[j].true_count) ===
                                          parseInt(trueTable[j].bill_count)
                                        ) {
  ...
  let chooseSettleByGroupControllerCallback = (request, response) => {
    ...
    if (cookieAvailable) {
      ...
      db.group.getSingleGroupWithBillDetails(
        group_id,
        user_id,
        (error, result) => {
          if (result) {
