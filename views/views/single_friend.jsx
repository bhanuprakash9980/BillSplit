...
class Single_Friend extends React.Component {
  render() {
    let datatarget = "#test"
    let amount = this.props.user_net>0?<span style={{color:"green"}}>{this.props.friend_details.name} owe you ₹{this.props.user_net}</span> : (this.props.user_net === 0? <span style={{color:"grey"}}>All cleared</span> : <span style={{color:"red"}}>You owe {this.props.friend_details.name} ₹{this.props.user_net*-1}</span>)

    let group_details = this.props.result.map(x=>{

        let amount = x.net>0?<p style={{color:"red"}}>You borrowed <span className="group-card-amount-only">₹{x.net.toFixed(2)}</span></p>: (x.net === 0? <p style={{color:"grey"}}>You owe nothing</p>:<p style={{color:"green"}}>You lent <span className="group-card-amount-only">₹{(x.net*-1).toFixed(2)}</span></p>)
    ...
    let modaltext = this.props.user_net>0? <span style={{color:"green"}}>₹{this.props.user_net}</span> : <span style={{color:"red"}}>₹{this.props.user_net*-1}</span>
