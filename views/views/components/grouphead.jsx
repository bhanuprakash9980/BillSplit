...
class GroupHead extends React.Component {
  render() {
    ...
    let text = parseFloat(this.props.user_total).toFixed(2) > 0? <p className="group-head-amount">You get back ₹{this.props.user_total}</p> : (parseFloat(this.props.user_total).toFixed(2) === 0?<p className="group-head-amount">No owings</p>: <p className="group-head-amount">You owe ₹{parseFloat(this.props.user_total*-1).toFixed(2)}</p>)
