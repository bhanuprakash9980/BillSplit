...
class BillCard extends React.Component {
  render() {
    ...
    let payment = ""
    let paidstatus = this.props.paid? "(Paid)":""
    if(user_id === payer_id){
        payment = <p className="amount-to-pay" style={{color:"green"}}>You lent <span>₹{((parseFloat(this.props.amount)-parseFloat(this.props.split_amount))).toFixed(2)}{paidstatus}</span></p>
    }else{
        if(parseFloat(this.props.split_amount)===0){
            payment = <p className="amount-to-pay"style={{color:"grey"}}>You are not involved</p>
        }else{
            payment = <p className="amount-to-pay"style={{color:"red"}}>You borrowed <span>₹{this.props.split_amount.toFixed(2)}{paidstatus}</span></p>
        }
    ...
    return (
      <div className="bill-card">
            ...
            <div className="bill-card-details">
                ...
                <p className="payer">{this.props.payer_name} paid ₹{this.props.amount}</p>
            </div>
