...
class BillHead extends React.Component {
  render() {
    ...
    let split_details = this.props.split_details.map(x=>{
        ...
        if(user_id === payee_id){
            if(user_id === payer_id){
                return <div className="bill-head-splitter-line"><img src={x.image}/><p>You paid <span style={{color:"green"}}>₹{(parseFloat(x.amount))}</span> </p></div>
                // and owe <span style={{color:"red"}}>₹{(parseFloat(x.split_amount))}</span>
            }else{
                if(x.split_amount === 0){
                    return <div className="bill-head-splitter-line"><img src={x.image}/><p>You are not involved</p></div>
                }else if(x.paid){
                    return <div className="bill-head-splitter-line"><img src={x.image}/><p>You owe <span style={{color:"red"}}>₹{(parseFloat(x.split_amount))}</span> (paid)</p></div>
                }else{
                    return <div className="bill-head-splitter-line"><img src={x.image}/><p>You owe <span style={{color:"red"}}>₹{(parseFloat(x.split_amount))}</span></p></div>
                }
            ...
            if(payee_id===payer_id){
                return <div className="bill-head-splitter-line"><img src={x.image}/><p>{x.name} paid <span style={{color:"green"}}>₹{(parseFloat(x.amount))}</span></p></div>
            }else{
                if(x.split_amount===0){
                    return <div className="bill-head-splitter-line"><img src={x.image}/><p>{x.name} is not involved</p></div>
                }
                else if(x.paid){
                    return <div className="bill-head-splitter-line"><img src={x.image}/><p>{x.name} owe <span style={{color:"red"}}>₹{(parseFloat(x.split_amount))}</span> (paid)</p></div>
                }else{
                    return <div className="bill-head-splitter-line"><img src={x.image}/><p>{x.name} owe <span style={{color:"red"}}>₹{(parseFloat(x.split_amount))}</span></p></div>
                }
      ...
      <div className="bill-head">
            <div className="bill-head-head-container"style={img_background}>
                ...
                <div className="bill-head-details">
                    ...
                    <p className="bill-head-amount">₹{this.props.bill_details.amount}</p>
