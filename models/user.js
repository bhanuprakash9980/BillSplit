...
module.exports = (dbPoolInstance) => {
  let registerPost = (registerInfo, callback) => {
    let query =
      'INSERT INTO users (name,password,image,mobile) VALUES ($1,$2,$3,$4) RETURNING *';
    let arr = [
      registerInfo.name,
  ...
  let getAllFriendsRelatedToUser = (user_id, callback) => {
    let query =
      'SELECT DISTINCT net_table.pay_to_id,users.name,users.image FROM net_table INNER JOIN users ON (users.id = net_table.pay_to_id) WHERE net_table.user_id = $1 ORDER BY pay_to_id DESC';
    let arr = [user_id];
    dbPoolInstance.query(query, arr, (error, queryResult) => {
      if (error) {
        callback(error, null);
  ...
  let getTransactionsForStatement = (user_id, callback) => {
    let query =
      'SELECT amount AS net,description as paid_to,name as group  FROM bills JOIN groups ON group_id=groups.id WHERE paid_by_user_id=$1 UNION SELECT net,users.name AS paid_to,groups.name AS group FROM net_table JOIN users ON pay_to_id=users.id JOIN groups ON group_id=groups.id WHERE user_id=$1 AND paid=true';
    let arr = [user_id];
    dbPoolInstance.query(query, arr, (error, queryResult) => {
      if (error) {
        callback(error, null);
